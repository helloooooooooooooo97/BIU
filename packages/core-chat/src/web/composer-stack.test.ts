...
describe('composer dock stacking above sticky user', () => {
  ...
  it('uses facet orange fill on clear-context only when there is history', () => {
    ...
    expect(css).toMatch(/\.project-chip-hist-bar\s*\{[^}]*color-mix\(in srgb, var\(--biu-tag, #d9730d\) 22%, transparent\)/s)
    expect(approvals).toMatch(/project-chip-hist-bar/)
    expect(css).toMatch(/\.chat-pane\s*\{[^}]*--dsw-ok:\s*#28C840/s)
    expect(css).toMatch(/\.chat-pane\s*\{[^}]*--dsw-danger:\s*#c4554d/s)
    expect(css).toMatch(/\.tool-call-status\.is-ok\s*\{[^}]*color:\s*#28C840/s)
    expect(css).not.toMatch(/\.tool-call-status\.is-ok\s*\{[^}]*color-mix\(in srgb, #28C840 22%, transparent\)/s)
    ...
    expect(css).toMatch(/\.tool-call-chevron\.is-ok\s*\{[^}]*color:\s*#28C840/s)
    expect(css).toMatch(/\.tool-call-chevron\.is-fail\s*\{[^}]*color:\s*#c4554d/s)
    expect(css).toMatch(/\.tool-call-head\.is-open\.is-ok::before[\s\S]*#28C840 22%/)
    ...
    expect(css).toMatch(/\.traj-usage-ring\.is-cache\s*\{[^}]*color:\s*#28C840/s)
    expect(css).toMatch(/\.traj-usage-ring\.is-hist\s*\{[^}]*color:\s*#FEBC2E/s)
    const usage = readFileSync(resolve(root, 'packages/core-chat/src/web/usage-inline.tsx'), 'utf8')
    expect(usage).toMatch(/#28C840/)
    expect(usage).toMatch(/#FEBC2E/)
