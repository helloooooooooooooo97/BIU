...
describe('panel collapse chevrons follow panel state', () => {
  ...
  it('declares inspector persist helpers once (no duplicate onOpen)', () => {
    const persistBlock = shell.match(
      /const persist = \(next: boolean\) => applyInspector\(next\)[\s\S]*?window\.addEventListener\('biu:inspector-toggle', onToggle\)/,
    )
    ...
    expect(persistBlock?.[0]).toContain('inspectorOpenRef.current')
  })
})
