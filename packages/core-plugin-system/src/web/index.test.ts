...
test('html and req page blocks register plugin id for slash', async () => {
  ...
  assert.match(html, /opacity: show \? 1 : 0/)
  assert.match(html, /pointerEvents: show \? 'auto' : 'none'/)
  assert.match(html, /deck: true/)
  assert.match(html, /丰富<br>排版/)
  assert.match(html, /直出 HTML/)
  assert.match(html, /沙箱 HTML/)
  assert.match(html, /Helvetica Neue/)
  assert.match(html, /boxShadow: '4px 4px 0 #111'/)
