...
test('tasksCollection maps /tasks rows with rolled-up usage', async () => {
  ...
  assert.deepEqual(spec.schema.columns, ['title', 'status', 'priority', 'difficulty', 'usage', 'createdBy', 'assignee', 'project', 'tags', 'dueAt'])
  ...
  assert.equal(spec.schema.fields.createdBy?.label, '创建人')
  assert.equal(spec.schema.fields.createdBy?.writable, false)
  assert.equal(spec.schema.fields.creator, undefined)
