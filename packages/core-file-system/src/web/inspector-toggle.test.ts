...
test('database extras sit after the record detail, not in the inspector', () => {
  ...
  assert.match(browser, /<BannerTitleActions[\s\S]*?<h1 className="fsdb-detail-title">\{activeView/)
  assert.match(detail, /<BannerTitleActions[\s\S]*?<h1 className="fsdb-detail-title">/)
  assert.doesNotMatch(browser, /<h1 className="fsdb-detail-title">\{activeView\?\.name \?\? title\}<\/h1>\s*<BannerTitleActions/)
  ...
  assert.match(style, /\.fsdb-banner-title-actions\{[^}]*position:absolute/)
  assert.match(style, /\.fsdb-detail-title-row:hover \.fsdb-banner-title-actions/)
  assert.doesNotMatch(style, /添加背景/)
  assert.doesNotMatch(detail, /添加背景/)
  ...
  assert.match(bannerUi, /forwardRef/)
