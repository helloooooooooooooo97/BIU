...
test('create record sits at the right of the toolbar with a blue label', () => {
  ...
  assert.match(css, /\.fsdb-row-check\{[^}]*opacity:1/)
...
test('database extras sit after the record detail, not in the inspector', () => {
  ...
  assert.match(bannerUi, /静态背景/)
  assert.match(bannerUi, /动态背景/)
  assert.match(bannerUi, /openTab\('html'\)/)
  assert.match(bannerUi, /openTab\('htmlframe'\)/)
  assert.match(bannerUi, /event\.stopPropagation\(\)/)
  assert.match(bannerUi, /setOpen\(true\)/)
