...
test('create record sits at the right of the toolbar with a blue label', () => {
  ...
  assert.match(css, /\.fsdb-row-check\{[^}]*translate\(calc\(-100% - 4px\),-50%\)/)
  assert.match(css, /\.fsdb-row-check\{[^}]*pointer-events:auto/)
  assert.match(css, /tbody tr:hover \.fsdb-row-check/)
  assert.match(css, /\.fsdb-check-rail\{[^}]*position:sticky/)
  assert.match(css, /\.fsdb-check-rail\{[^}]*margin:0 0 0 calc\(var\(--fsdb-check-gutter\) \* -1\)/)
  assert.doesNotMatch(css, /\.fsdb-check-rail\{[^}]*left:0/)
  assert.match(css, /\.fsdb-page \.tasks-table-wrap\{[^}]*padding-left:var\(--fsdb-check-gutter\)/)
  assert.doesNotMatch(css, /\.fsdb-page \.tasks-table-wrap\{[^}]*margin-left:calc\(var\(--fsdb-check-gutter\) \* -1\)/)
  ...
  assert.match(css, /td:has\(> \.fsdb-row-check\),[\s\S]*th:has\(> \.fsdb-row-check\)\{[^}]*overflow:visible/)
  ...
  assert.match(browser, /col.key === titleColKey \? <RowCheck id=\{row\.id\} \/> : null/)
  assert.match(browser, /className="fsdb-check-rail"/)
  assert.match(browser, /<RowCheck ids=\{pickableIds\} \/>/)
  assert.doesNotMatch(browser, /titleColKey \? <RowCheck ids=\{pickableIds\}/)
