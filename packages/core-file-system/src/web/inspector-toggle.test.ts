...
test('create record sits at the right of the toolbar with a blue label', () => {
  ...
  assert.match(css, /\.fsdb-row-check\{[^}]*position:relative/)
  assert.doesNotMatch(css, /\.fsdb-row-check\{[^}]*translate\(-100%/)
  ...
  assert.doesNotMatch(css, /th:has\(\.fsdb-row-check\)::before/)
  assert.match(browser, /function tableBodyRows/)
  assert.match(browser, /markCellOn/)
  assert.doesNotMatch(browser, /<TableBodyRows/)
  assert.match(browser, /className="fsdb-title-lead"/)
  assert.match(browser, /<RowCheck id=\{row\.id\} \/>/)
  assert.match(browser, /col.key === titleColKey \? <RowCheck ids=\{pickableIds\} \/> : null/)
  assert.match(browser, /className="tasks-th"[\s\S]*titleColKey \? <RowCheck ids=\{pickableIds\}/)
